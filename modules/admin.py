...
def render_admin_panel():
    ...
    with tab1:
        ...
        if st.button("Add Branch"):
            if new_code and new_name:
                exists = supabase.table("branches").select("code").eq("code", new_code.upper()).execute().data
                if exists:
        ...
        if st.button("Check Branch"):
            if not remove_code:
                ...
                # Query slips linked to this branch
                slips_resp = supabase.table("slips").select("*").eq("branch_code", remove_code.upper()).execute()
                slips = slips_resp.data or []

                if slips:
                    st.warning(f"Branch '{remove_code.upper()}' has {len(slips)} slip(s) linked. Deleting the branch will also delete these slips.")
                    st.dataframe(slips)

                    if st.button("Confirm Delete Anyway"):
                        try:
                            supabase.table("slips").delete().eq("branch_code", remove_code.upper()).execute()
                            branch_resp = supabase.table("branches").delete().eq("code", remove_code.upper()).execute()
                            st.success(f"Branch '{remove_code.upper()}' and its linked slips deleted successfully.")
                            st.rerun()
                        except Exception as e:
                    ...
                    try:
                        ...
                        st.error(f"Error removing branch: {str(e)}")
    ...
    with tab2:
        ...
        if code:
            result = supabase.table("branches").select("*").eq("code", code.upper()).single().execute()
            branch = result.data
            if not branch:
                    ...
                    for rider in riders:
                        ...
                        with col2:
                            if st.button(f"Remove {rider}", key=f"rm_{rider}"):
                                ...
                                supabase.table("branches").update({"riders": updated}).eq("code", code.upper()).execute()
